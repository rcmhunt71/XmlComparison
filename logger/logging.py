...
class Logger:
    ...
    * reapplies the handlers to the root, which will cause the handlers
    ...
    def list_loggers(self) -> str:
        ...
        if pretty_table:
            ...
            for data in self._list_loggers():
                table.add_row(data)
            table_str = table.get_string(title='Loggers')

        # Build table manually since PrettyTable is not available
        else:

            # Define table parameters
            border_char = '='
            intercol_space = ' ' * 3
            max_lvl_len = len(level)
            max_lvl_len += 4 if (max_lvl_len % 2 == 0) else 3
            rows = self._list_loggers()
            max_logger_name_len = max([len(entry) for entry, _ in rows])

            # Define table format
            border = f"{border_char * max_logger_name_len}{intercol_space}{border_char * max_lvl_len}\n"
            table_row_format = f"{{var_1:<{max_logger_name_len}}}{intercol_space}{{var_2:^{max_lvl_len}}}\n"

            # Build table
            table_str = f"{border}{table_row_format.format(var_1=child, var_2=level)}{border}"
            for data in self._list_loggers():
                table_str += table_row_format.format(var_1=data[0], var_2=data[1])
            table_str += border
    ...
    def _method(self) -> typing.Dict[str, typing.Any]:
        ...
        filename = str(os.path.abspath(frame_info[1]).split(f'{self.project}{os.path.sep}')[-1])
...
if __name__ == '__main__':  # pragma: no cover
    ...
    LOG = Logger(default_level=Logger.DEBUG,
